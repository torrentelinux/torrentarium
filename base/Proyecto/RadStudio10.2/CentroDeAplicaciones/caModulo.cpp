...
__fastcall TfrmPrincipal::TfrmPrincipal(TComponent* Owner) : TForm(Owner)
{
   ...
   aplics[5] = lblComprobar->Caption;
   aplics[6] = lblComprobarJDev->Caption;

   // Oracle JDeveloper, básico: jdev_java_122140.zip
...
void __fastcall TfrmPrincipal::ButtonGroup1Items0Click(TObject *Sender)
{
   ...
   var_usrprofile = _tgetenv(L"USERPROFILE");
...
//---------------------------------------------------------------------------
void __fastcall TfrmPrincipal::lblComprobarMouseEnter(TObject *Sender)
{
   lblComprobar->Caption = L"► " + lblComprobar->Caption;

   lblComprobar->Font->Color = clBlue;
   lblComprobar->Font->Style = TFontStyles() << fsUnderline;
}
//---------------------------------------------------------------------------
void __fastcall TfrmPrincipal::lblComprobarMouseLeave(TObject *Sender)
{
   lblComprobar->Caption = aplics[5];

   lblComprobar->Font->Color = clWindowText;
   lblComprobar->Font->Style = TFontStyles();
}
//---------------------------------------------------------------------------
void __fastcall TfrmPrincipal::lblComprobarClick(TObject *Sender)
{
   // Abre una consola de textos para verificar la presencia de Java en el sistema.
	int status = -1;

     sbBarraDeEstado->SimpleText = L"Comprobando la presencia de Java en el sistema...";

     // Abre la aplicación cmd con sus parámetros
     status = _tsystem(L"java.exe --version & pause");
     if(status != 0)
     {
          String condicion;

       condicion = L"Error " + String(errno) + L" en: consola.";
       ShowMessage(condicion);
     ...
     {
          TCHAR *encontrado = NULL;

       Sleep(150);
       sbBarraDeEstado->SimpleText = L"Aplicación iniciada.";

       encontrado = _tsearchpath(L"java.exe");
       if(encontrado)
       {
                String resultado;

         sbBarraDeEstado->SimpleText = L"Ok.";
         resultado = L"Encontrado:\n" + String(encontrado);
         ShowMessage(resultado);
       }
       else
       {
         sbBarraDeEstado->SimpleText = L"No se encuentra instalado Java.";
         ShowMessage(L"No encontrado.");
       }
     }
}
//---------------------------------------------------------------------------
void __fastcall TfrmPrincipal::lblComprobarJDevClick(TObject *Sender)
{
   // Verifica la presencia de Oracle JDeveloper IDE en el sistema.
	int status = -1;
        TCHAR *jdev = L"jdeveloper.exe";
        String lugar = dlgAbrirDir->FileName + L"\\jdeveloper\\";
        wostringstream encontrado;
        wostringstream a_buscar;

   sbBarraDeEstado->SimpleText = L"Comprobando la presencia de Oracle JDeveloper IDE en el sistema...";

   a_buscar << lugar << jdev << ends;

   status = _taccess(a_buscar.str().c_str(), 0);
   if(status == 0)
   {
       encontrado << L"Encontrado en:" << endl << a_buscar.str() << ends;
       ShowMessage(encontrado.str().c_str());
   }
   else
   {
       encontrado << L"No encontrado en:" << endl << a_buscar.str() << ends;
       ShowMessage(encontrado.str().c_str());
   }

   // Busca el fichero product.conf a partir del subdirectorio especificado por %APPDATA%
   status = _tsystem(L"pushd %APPDATA% & echo [ Fichero de configuración de Oracle JDeveloper ] & echo. & forfiles /s /m product.conf /c \"cmd /c echo @path\"|find /i \"jdev\" & popd & pause");
   if(status != 0)
   {
          String condicion;
   ...
   {
       sbBarraDeEstado->SimpleText = L"Ok.";
       ShowMessage(L"Se ha comprobado la presencia de Oracle JDeveloper.");
   }
}
//---------------------------------------------------------------------------
void __fastcall TfrmPrincipal::lblComprobarJDevMouseEnter(TObject *Sender)
{
   lblComprobarJDev->Caption = L"► " + lblComprobarJDev->Caption;

   lblComprobarJDev->Font->Color = clBlue;
   lblComprobarJDev->Font->Style = TFontStyles() << fsUnderline;
}
//---------------------------------------------------------------------------
void __fastcall TfrmPrincipal::lblComprobarJDevMouseLeave(TObject *Sender)
{
   lblComprobarJDev->Caption = aplics[6];

   lblComprobarJDev->Font->Color = clWindowText;
   lblComprobarJDev->Font->Style = TFontStyles();
}
//---------------------------------------------------------------------------
